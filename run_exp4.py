"""
Experiment 4: damped_pendulum_4, random sampling, larger model (hidden=200, depth=3), 500 epochs.

Diagnosis from experiments 1-3:
- Exp 1 (MSD):        Lyapunov 100%, both models excellent  → implementation correct
- Exp 2 (p4 traj):   MSE~9-13, both diverge on rollout    → trajectory data too sparse
- Exp 3 (p4 random): MSE stable=16.77 ≈ baseline=16.28   → similar fit, stable 27x worse on rollout

Key insight: stable and baseline have nearly identical MSE, so the Lyapunov projection
is not the bottleneck — the model is underpowered/undertrained for 8D chaotic dynamics.
Original paper used 1000 epochs; we used 200. Increasing to 500 epochs and doubling
model capacity should close the rollout gap.
"""
...
# ── Load existing random p4 data (already generated by run_experiments.py) ──
...
# Same initial conditions as trajectory experiment for fair comparison
...
print("\n\nDone.")
print(f"  deriv MSE:    stable={summary4['derivative_mse_stable']:.4g}  baseline={summary4['derivative_mse_baseline']:.4g}")
print(f"  rollout err:  stable={summary4['final_rollout_error_stable']:.4g}  baseline={summary4['final_rollout_error_baseline']:.4g}")
print(f"  Lyapunov:     max_viol={summary4['lyapunov_max_violation']:.4g}  frac_sat={summary4['lyapunov_fraction_satisfied']:.4f}")
